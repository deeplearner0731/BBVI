...
def theta_(x,beta,gamma):
    n=x.shape[0]
    x=np.column_stack((np.ones(n),x))
    val1=np.tensordot(gamma,x,(1,1))
    val2=1.0/(1+np.exp(-val1))
    theta=beta[0]+np.dot(beta[1::].reshape(1,kn),val2)
    return theta

def train_accuracy(False,vr_beta_mean,vr_beta_rho,vr_gamma_mean,vr_gamma_rho):
    Beta,Gamma=q_simulate(10000,vr_beta_mean,vr_beta_rho,vr_gamma_mean,vr_gamma_rho)
    Theta=np.array(list(map(lambda x:theta_(X,Beta[:,x],Gamma[:,:,x]),range(Beta.shape[1]))))  
    sTheta=1.0/(1+np.exp(-Theta))
    stheta=np.apply_along_axis(np.mean, 0, sTheta)
    Y_est=np.array([0 if x<0.5 else 1 for x in stheta[0]])
    Y_est=Y_est.reshape((len(Y_est),))
    return sum(Y==Y_est)*1.0/len(Y)

def test_accuracy(False,vr_beta_mean,vr_beta_rho,vr_gamma_mean,vr_gamma_rho):
    Beta,Gamma=q_simulate(10000,vr_beta_mean,vr_beta_rho,vr_gamma_mean,vr_gamma_rho)
    Theta=np.array(list(map(lambda x:theta_(X_test,Beta[:,x],Gamma[:,:,x]),range(Beta.shape[1]))))  
    sTheta=1.0/(1+np.exp(-Theta))
    stheta=np.apply_along_axis(np.mean, 0, sTheta)
    Y_est=np.array([0*1.0 if x<0.5 else 1 for x in stheta[0]])
    return sum(Y_test==Y_est)*1.0/len(Y_test)
